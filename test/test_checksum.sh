...
(cd "$WORK" && "$SAR" p archive.sar src/hello.txt 2>/dev/null)
out="$WORK/t1" && mkdir "$out"
(cd "$out" && "$SAR" u "$WORK/archive.sar" 2>/dev/null)
...
if (cd "$out" && "$SAR" u "$WORK/corrupt_data.sar" 2>/dev/null); then
...
if (cd "$out" && "$SAR" u "$WORK/corrupt_hdr.sar" 2>/dev/null); then
...
if (cd "$out" && "$SAR" u "$WORK/corrupt_cksum.sar" 2>/dev/null); then
...
(cd "$WORK" && "$SAR" p -j 2 mt.sar src/hello.txt 2>/dev/null)
out="$WORK/t5" && mkdir "$out"
(cd "$out" && "$SAR" u "$WORK/mt.sar" 2>/dev/null)
if [ "$(cat "$out/src/hello.txt" 2>/dev/null)" = "hello checksum" ]; then
...
if (cd "$out" && "$SAR" u "$WORK/mt_corrupt.sar" 2>/dev/null); then
...
(cd "$WORK" && "$SAR" p link.sar src/link 2>/dev/null)
out="$WORK/t7" && mkdir "$out"
(cd "$out" && "$SAR" u "$WORK/link.sar" 2>/dev/null)
...
if (cd "$out" && "$SAR" u "$WORK/link_corrupt.sar" 2>/dev/null); then
...
(cd "$WORK" && "$SAR" p - src/link 2>/dev/null > link_pipe.sar)
out="$WORK/t9" && mkdir "$out"
(cd "$out" && cat "$WORK/link_pipe.sar" | "$SAR" u - 2>/dev/null)
if [ "$(readlink "$out/src/link" 2>/dev/null)" = "hello.txt" ]; then
...
if (cd "$out" && cat "$WORK/link_pipe_corrupt.sar" | "$SAR" u - 2>/dev/null); then
